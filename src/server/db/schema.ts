 ...
 */
export const mysqlTable = mysqlTableCreator((name) => `tektok_${name}`);
...
export const postUserRelations = relations(posts, ({ one }) => ({
  author: one(users, {
    fields: [posts.userId],
    references: [users.id],
  }),
...
export const commentUserRelations = relations(comments, ({ one }) => ({
  author: one(users, {
